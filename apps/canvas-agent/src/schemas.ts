import { z } from "zod";
import { CANVAS_AGENT_TOOL_NAMES, type CanvasAgentToolName } from "@ai-manju/canvas-agent-protocol";
...
export const toolNames = CANVAS_AGENT_TOOL_NAMES;
export type ToolName = CanvasAgentToolName;
...
export const toolInputSchemas = {
    ...
    canvas_search_assets: z.object({ keyword: z.string().optional(), assetType: z.enum(["image", "video", "audio"]).optional(), category: z.enum(["character", "environment", "costume", "prop", "ui", "reference", "other"]).optional(), sourceType: z.string().optional(), limit: z.number().int().min(1).max(50).optional() }),
    canvas_add_assets: z.object({ assetIds: z.array(z.string()).min(1).max(20), x: z.number().optional(), y: z.number().optional(), direction: z.enum(["row", "column"]).optional(), gap: z.number().optional() }),
    canvas_list_jobs: z.object({ statuses: z.array(z.enum(["queued", "running", "succeeded", "failed", "canceled"])).optional(), types: z.array(z.string()).optional(), limit: z.number().int().min(1).max(100).optional() }),
    canvas_cancel_job: z.object({ jobId: z.string() }),
...
export const toolDescriptions: Record<ToolName, string> = {
    ...
    canvas_search_assets: "搜索当前 workspace 资产库，可按关键词、类型、分类和来源筛选。",
    canvas_add_assets: "把指定资产添加到当前画布，支持横向或纵向排列。",
    canvas_list_jobs: "读取当前 workspace 的渲染任务，可按状态和任务类型筛选。",
    canvas_cancel_job: "取消指定渲染任务。",
