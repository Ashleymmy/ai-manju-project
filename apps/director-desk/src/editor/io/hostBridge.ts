...
const DIRECTOR_DESK_LIFECYCLE_ID = typeof globalThis.crypto?.randomUUID === "function"
  ? globalThis.crypto.randomUUID()
  : `director-lifecycle-${Date.now()}-${Math.random().toString(36).slice(2)}`;
...
export function postDirectorDeskReadyToHost() {
  postDirectorDeskMessageToHost({
    ...
    payload: { lifecycleId: DIRECTOR_DESK_LIFECYCLE_ID },
  });
}
