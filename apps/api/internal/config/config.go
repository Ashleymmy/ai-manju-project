...
type Config struct {
	...
	// Billing gates the credit-ledger wiring (reserve on enqueue, settlement
	// reconciler, register bonus). Default off: 不破坏现有公开行为，先上线代码
	// 再按环境开启。
	BillingEnabled              bool
	BillingReconcileIntervalSec int
	// BillingSchedulerIntervalSec drives the WP-M4 月发/过期清扫循环。
	BillingSchedulerIntervalSec int
}
...
func Load() Config {
	...
	cfg := Config{
		...
		BillingEnabled:              getBoolEnv("BILLING_ENABLED", false),
		BillingReconcileIntervalSec: getIntEnv("BILLING_RECONCILE_INTERVAL_SECONDS", 15),
		BillingSchedulerIntervalSec: getIntEnv("BILLING_SCHEDULER_INTERVAL_SECONDS", 60),
	}
