...
export function canvasDirectorOutputExists(
  snapshot: Record<string, unknown>,
  nodeId: string,
  outputKey: string
) {
  const nodes = Array.isArray(snapshot.nodes)
    ? snapshot.nodes.filter(isRecord)
    : [];
  const directorNode = nodes.find(
    node => stringValue(node.id) === nodeId && nodeKind(node) === "director"
  );
...
export function applyDirectorFrameToCanvasSnapshot(input: {
  ...
  const sourceNodes = Array.isArray(input.snapshot.nodes)
    ? input.snapshot.nodes
    : [];
  const nodes = sourceNodes.filter(isRecord);
  const directorIndex = nodes.findIndex(
    node =>
      stringValue(node.id) === input.identity.nodeId &&
      nodeKind(node) === "director"
  );
  if (directorIndex < 0) throw new Error("未找到对应的导演台画布节点");
  const directorNode = nodes[directorIndex];
  const metadata = isRecord(directorNode.metadata)
    ? { ...directorNode.metadata }
    : {};
  ...
  const updatedDirector = {
    ...
    metadata: {
      ...
      directorOutputNodeIds: [
        ...stringArray(metadata.directorOutputNodeIds),
        outputNodeId,
      ],
  ...
  const nextNodes = nodes
    .map((node, index) => (index === directorIndex ? updatedDirector : node))
    .concat(outputNode);
  const sourceEdges = (
    Array.isArray(input.snapshot.edges)
      ? input.snapshot.edges
      : Array.isArray(input.snapshot.connections)
        ? input.snapshot.connections
        : []
  ).filter(isRecord);
  const hasConnection = sourceEdges.some(
    edge =>
      edgeFrom(edge) === input.identity.nodeId && edgeTo(edge) === outputNodeId
  );
  const nextEdges = hasConnection
    ? sourceEdges
    : [
        ...sourceEdges,
        {
...
function nodePosition(node: Record<string, unknown>) {
  ...
  return {
    x: numberValue(node.x, numberValue(position.x)),
    y: numberValue(node.y, numberValue(position.y)),
  };
}
