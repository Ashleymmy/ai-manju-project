...
describe("video API", () => {
  ...
  it.each(["480p", "768p"])("submits all nine H3 references with fixed %s settings", async resolution => {
    ...
    await createVideoGenerationTask({ ...config, model, seconds: "30", size: "9:16" }, "镜头", refs);
    ...
    expect(h3VideoSettings(model)).toEqual({ durations: Array.from({ length: 15 }, (_, i) => i + 1), resolutions: [resolution], ratios: ["16:9", "9:16"] });
