...
export function CanvasGenerationPrice({ node, edit = false, panorama = false }: { node: CanvasNodeData; edit?: boolean; panorama?: boolean }) {
  ...
  if (mode !== "image" && mode !== "video") return <GenerationPrice kind="free" />;
  if (mode === "video") {
    const config = videoConfigFromNode(node, context.videoModel);
    return <GenerationPrice kind="video" model={config.model} seconds={config.seconds} resolution={config.resolution} audio={config.generateAudio} referenceVideos={retry ? node.metadata?.videoReferenceInputs?.items.filter(item => item.type === "video").length ?? 0 : refs.filter(ref => ref.kind === "video").length} />;
  }
  ...
  return <GenerationPrice model={modelFromNode(node, context.imageModel)} {...canvasImageGenerationSettings(node, panorama ? "2:1" : undefined)} references={edit ? 1 : retry ? node.metadata?.referenceInputs?.length ?? 0 : refs.filter(ref => ref.kind === "image").length} tasks={edit ? 1 : tasks} />;
}
