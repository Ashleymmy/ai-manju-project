...
import { defaultMediaMimeType, isReadableMediaSource, mediaFileName, mediaKindFromNode, mediaKindLabel } from "../domain/nodeUtils";
...
export async function archiveCanvasMediaAsset(input: {
  ...
  if (existingAssetId && sourceScope === scope) {
    ...
    return services.updateAssetMetadata(existingAssetId, { folder_id: folderId, category }, scope);
  }
  let objectUrl = "";
  try {
    ...
    const fileName = mediaFileName(node.title, kind, contentType);
    return await services.uploadAsset(new File([blob], fileName, { type: contentType }), {
      type: kind, name: fileName, folder_id: folderId, category,
      source_type: "canvas", source_project_id: input.projectId, source_project_name: input.projectTitle,
      source_node_id: node.id,
      source_metadata: JSON.stringify({ node_id: node.id, operation: existingAssetId ? "cross_scope_copy" : "archive" }),
    }, scope);
  } finally {
