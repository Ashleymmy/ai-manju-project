...
export function parseCanvasFragmentPackage(
  ...
  const groups = Array.isArray(source.groups)
    ? source.groups.map((item) => normalizeFragmentGroup(item, createGroupId))
    : [];
  return {
    version: 1,
...
export function importCanvasFragmentPackage(input: {
  ...
  const nodes = input.fragment.nodes.map((source) => rewriteFragmentNode(
    source,
    input.assets,
    nodeIds,
    input.scope,
    dx,
    dy,
    input.createDirectorInstanceId,
  ));
...
function rewriteFragmentNode(
  ...
  if (typeof metadata.directorInstanceId === "string") {
    metadata.directorInstanceId = createDirectorInstanceId();
...
function normalizeFragmentGroup(
  value: unknown,
  createGroupId: () => string,
): CanvasFragmentGroup {
  ...
  return {
    ...group,
    id: stringValue(group.id) || createGroupId(),
    nodeIds,
