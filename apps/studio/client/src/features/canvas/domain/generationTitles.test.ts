...
describe("generated media node names", () => {
  it.each([
  ...
  ] as const)("names completed %s results by project without changing downloadable formats", (kind, extension, mime) => {
    ...
    nodes = ensureUniqueCanvasNodeTitles(nodes, [], "测试");
    expect(nodes.map(node => node.title)).toEqual([`测试${kind}-1`, `测试${kind}-2`, `测试${kind}-3`]);
    ...
    expect(mediaFileName(nodes[0].title, kind, mime)).toBe(`测试${kind}-1.${extension}`);
  });

  it("ignores prompt and provider filenames when naming generated images", () => {
    ...
    nodes = ensureUniqueCanvasNodeTitles(nodes, [], "测试");
    expect(nodes.map(node => node.title)).toEqual(["测试image-1", "测试image-2", "测试image-3"]);
  });
});
