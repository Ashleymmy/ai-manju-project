...
export function ModelPricingTable({ prices }: { prices: MemberPricing["model_prices"] }) {
  ...
  for (const [model, resolutions] of Object.entries(prices.images)) {
    for (const [resolution, qualities] of Object.entries(resolutions)) {
      qualities.forEach((price, index) => rows.push([model, qualities.length === 1 ? resolution.toUpperCase() : `${resolution.toUpperCase()} / ${prices.qualities[index]}`, `${price} / 张`]));
    }
  }
  ...
  return <>
    ...
    <p className="member-tip">不支持画质档位的图片模型按分辨率计价。尺寸为自动，或支持画质档位的模型选择自动画质时，生成单价沿用基础兜底价；参考图仍逐张加收附加费，每个输出均计入。</p>
