// SHA-256 constants and word operations from FIPS 180-4. This fallback keeps
// opaque recovery identifiers available on HTTP, where SubtleCrypto is absent.
...
export async function sha256Hex(text: string): Promise<string> {
  ...
  if (globalThis.crypto?.subtle) {
    ...
    } catch { /* Browser policy may expose SubtleCrypto but deny access. */ }
  }
  const padded = new Uint8Array(Math.ceil((bytes.length + 1 + LENGTH_BYTES) / BLOCK_BYTES) * BLOCK_BYTES);
  padded.set(bytes);
  padded[bytes.length] = 0x80;
  const view = new DataView(padded.buffer);
  const bitLength = bytes.length * 8;
  view.setUint32(padded.length - LENGTH_BYTES, Math.floor(bitLength / 0x100000000));
  view.setUint32(padded.length - WORD_BYTES, bitLength >>> 0);
  ...
  for (let offset = 0; offset < padded.length; offset += BLOCK_BYTES) {
    for (let i = 0; i < 16; i++) words[i] = view.getUint32(offset + i * WORD_BYTES);
