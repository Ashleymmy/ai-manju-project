...
import { AssetLibraryView, ComicAssetsView, ImageWorkbenchView, ProfileView, PromptLibraryView, SkillLibraryView, TagLibraryView } from "./RealFeatureViews";
...
const pageTitles: Record<string, { code: string; title: string; subtitle: string }> = {
  ...
  "/skills": { code: "SKILLS / LIVE", title: "技能库", subtitle: "提示词优化技能与画布节点实时联动，启停即生效。" },
...
/** 折线侧栏导航（LineSidebar 风格）：刻度线 + 序号 + 文字，指针接近时行级联动
    （marker 变长 / 文字右移 / 提亮，rAF 指数平滑）；分组可折叠展开子级菜单。
    每行的 --effect(0..1) 由 JS 驱动，CSS 只负责映射到视觉。 */
const RAIL_GROUPS_KEY = "ai-manju:rail-open-groups";

function LineNav({ groups, currentPath }: { groups: Array<{ id: string; title: string; items: NavItem[] }>; currentPath: string }) {
  const [openGroups, setOpenGroups] = useState<string[]>(() => {
    try {
      const saved = JSON.parse(localStorage.getItem(RAIL_GROUPS_KEY) || "null");
      if (Array.isArray(saved)) return saved.filter((item) => typeof item === "string");
    } catch { /* 忽略损坏的本地状态 */ }
    return groups.map((group) => group.id); // 默认全部展开
  });
  ...
  const runFrame = useCallback((now: number) => {
    ...
    lastRef.current = now;
    const k = 1 - Math.exp(-dt / 0.1); // smoothing = 100ms
    let moving = false;
    rowRefs.current.forEach((el, index) => {
      if (!el) return;
      const target = Math.max(targetsRef.current[index] || 0, el.dataset.active === "1" ? 1 : 0);
  ...
  // 当前页所在分组始终保持展开
  useEffect(() => {
    const owner = groups.find((group) => group.items.some((item) => item.href === currentPath));
    if (owner && !openGroups.includes(owner.id)) {
      setOpenGroups((current) => {
        const next = [...current, owner.id];
        try { localStorage.setItem(RAIL_GROUPS_KEY, JSON.stringify(next)); } catch { /* 忽略 */ }
        return next;
      });
    }
  ...
  const handlePointerMove = (event: React.PointerEvent) => {
    ...
    rowRefs.current.forEach((el, index) => {
      ...
      targetsRef.current[index] = p * p * (3 - 2 * p); // smooth 衰减
    });
    startLoop();
  };
  ...
  return (
    <ul ref={listRef} className="line-nav" onPointerMove={handlePointerMove} onPointerLeave={handlePointerLeave}>
      {groups.map((group, groupIndex) => {
        const open = openGroups.includes(group.id);
        const parentRow = ++rowCounter;
        return (
          <li key={group.id} className="ln-group">
            <button type="button" className="ln-row ln-parent" data-active="0" ref={(el) => { rowRefs.current[parentRow] = el; }} onClick={() => toggleGroup(group.id)} aria-expanded={open}>
              <span className="ln-marker" aria-hidden="true" />
              <span className="ln-index">{String(groupIndex + 1).padStart(2, "0")}</span>
              <span className="ln-label">{group.title}</span>
              <ChevronRight size={13} className={`ln-chev ${open ? "open" : ""}`} />
            </button>
            <div className={`ln-children ${open ? "open" : ""}`}>
              <div className="ln-children-inner">
                {group.items.map((item, itemIndex) => {
                  ...
                  return (
                    <Link key={item.href} href={item.href} title={item.label} className="ln-row ln-item" data-active={active ? "1" : "0"} ref={(el) => { rowRefs.current[itemRow] = el; }} aria-current={active ? "page" : undefined}>
                      <span className="ln-marker" aria-hidden="true" />
                      <span className="ln-index">{String(groupIndex + 1).padStart(2, "0")}.{String(itemIndex + 1).padStart(2, "0")}</span>
...
export default function Home() {
  ...
  const renderPage = () => {
    ...
    if (path === "/skills") return <SkillLibraryView />;
  ...
  return <div className={`studio-app ${collapsed ? "rail-collapsed" : ""}`}><SideRail currentPath={path} collapsed={collapsed} onCollapse={() => setCollapsed((value) => !value)} data={shellData} /><SideFootCard data={shellData} /><main className="main-stage"><TopBar path={path} runningJobs={shellData.jobs.total} /><AnnouncementBanner />{renderPage()}</main><ReleaseNotesDialog /></div>;
}
