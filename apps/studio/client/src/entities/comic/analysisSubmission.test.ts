...
it("seals a missing multipart claim and only a later user action creates a fresh key", async () => {
  ...
  expect(String(vi.mocked(fetch).mock.calls[2][0])).toContain(`/receipts/comic_analysis/${submittedKey}/reconcile`);
  expect(vi.mocked(fetch).mock.calls[2][1]?.method).toBe("POST");
  vi.mocked(fetch).mockResolvedValueOnce(json(202, { session: { id: "analysis", status: "active" }, revisions: [] }));
  await createComicAnalysisSession(input, file);
  const nextKey = (vi.mocked(fetch).mock.calls[4][1]?.headers as Record<string, string>)["Idempotency-Key"];
  expect(nextKey).not.toBe(submittedKey);
});

it("retains the original identity when reconciliation metadata is unbound", async () => {
  const file = new File([input.source_text], "script.txt");
  vi.mocked(fetch).mockResolvedValueOnce(json(504, null, false))
    .mockResolvedValueOnce(json(404, null, false))
  ...
  vi.mocked(fetch).mockResolvedValueOnce(json(200, { status: "ready", session_id: "analysis" }))
