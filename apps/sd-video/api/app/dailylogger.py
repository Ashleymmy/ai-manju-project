...
class SafeJSONFormatter(logging.Formatter):
    def format(self, record):
        ...
        message = re.sub(r"Bearer\s+\S+|eyJ[\w-]+\.[\w-]+\.[\w-]+", "[token]", message)
        return json.dumps({"level": record.levelname, "logger": record.name, "message": message[:4000]}, ensure_ascii=False)
...
def get_logger(name: str = "daily_logger", log_dir: str = None) -> logging.Logger:
    ...
    if os.getenv("APP_ENV") == "production":
        ...
        console.setFormatter(SafeJSONFormatter())
