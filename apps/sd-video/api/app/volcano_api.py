...
from app.providers.seedance import (
    LEGACY_PROXY,
...
class VolcanoVideoAPI:
    ...
    def _is_seedance20_model(model: Optional[str]) -> bool:
        return bool(model and model in settings.SEEDANCE20_MODEL_IDS)
    ...
    @staticmethod
    def _raise_provider_error(exc: SeedanceProviderError) -> None:
        error_type = VolcanoGatewayError if isinstance(exc, SeedanceProviderGatewayError) else VolcanoAPIError
        raise error_type(
            str(exc),
    ...
    def _safe_payload_for_log(payload: dict) -> dict:
        ...
        if "content" in payload and isinstance(payload["content"], list):
            ...
            for item in payload["content"] or []:
                ...
                if item_type == "text":
                    ...
                    summary["url_kind"] = "asset" if url.startswith("asset://") else "signed_or_external"
        ...
        for key in ["image_urls", "video_urls", "audio_urls"]:
            if key in payload:
                ...
                safe_payload[f"{key}_kinds"] = [
                    "asset" if str(url).startswith("asset://") else "signed_or_external"
                    for url in urls[:3]
                ]
        ...
        if self._is_seedance20_model(model):
            ...
            if not provider.configured():
                raise VolcanoAPIError(
                    f"Seedance provider is not configured: {provider.name}",
            ...
            # 将前端内部 Token 格式 (@图片N_name) 统一转换为官方中文格式（图片N）
            original_tokens = re.findall(r'@(?:图片|视频|音频)\d+(?:_[^\s]*)?', cleaned_prompt)
            cleaned_prompt = re.sub(r'@图片(\d+)(?:_[^\s]*)?', r'图片\1', cleaned_prompt)
            cleaned_prompt = re.sub(r'@视频(\d+)(?:_[^\s]*)?', r'视频\1', cleaned_prompt)
            cleaned_prompt = re.sub(r'@音频(\d+)(?:_[^\s]*)?', r'音频\1', cleaned_prompt)
            ...
            try:
                result = await provider.create_video_task(payload)
                logger.info(
                    "[Volcano API] Seedance task created | provider=%s | task_id=%s",
                    provider.name,
                    result.get("id"),
                )
                return result
    ...
    async def query_task(
        ...
        if self._is_seedance20_model(model):
            ...
            for attempt in range(max_retries):
                ...
                except SeedanceProviderGatewayError as exc:
            ...
            if last_provider_error:
                self._raise_provider_error(last_provider_error)

        base_url = self.base_url
        endpoint = f"/api/v3/contents/generations/tasks/{task_id}"
        headers = self.headers
        ...
        for attempt in range(max_retries):
            try:
                ...
                if status_code in (502, 503, 504):
                    logger.warning(f"[Volcano API] 查询重试({attempt+1}/{max_retries}) | 状态: {status_code} | URL: {e.request.url}")
                    last_exc = e
                    if attempt < max_retries - 1:
                ...
                # 余额不足（402）→ 抛出携带 error_code 的 VolcanoAPIError，让上层能识别并终止轮询
                if status_code == 402:
                    error_code = self._extract_error_code(e) or "InsufficientBalance"
                    raise VolcanoAPIError(msg, error_code=error_code, http_status=402) from e

                if status_code in (502, 503, 504):
                    raise VolcanoGatewayError(msg, http_status=status_code) from e

                # 其他业务错误
                error_code = self._extract_error_code(e)
                raise VolcanoAPIError(msg, error_code=error_code, http_status=status_code) from e
