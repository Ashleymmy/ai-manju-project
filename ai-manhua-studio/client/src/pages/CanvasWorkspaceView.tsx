              ...
              <div className="inspector-block inspector-compact-params">
                <div className="parameter-row">
                  ...
                  <select value={selectedGenerationMode} onChange={(event) => {
                    const mode = event.target.value as CanvasGenerationMode;
                    ...
                    {(["text", "image", "video", "audio"] as const).map((mode) => <option key={mode} value={mode}>{generationModeLabel(mode)}</option>)}
                  </select>
                </div>
                <div className="parameter-row">
                  <span>模型</span>
                  <select
                    ...
                    onChange={(event) => {
                      if (selectedGenerationMode === "text") setTextModel(event.target.value);
                      if (selectedGenerationMode === "image") setImageModel(event.target.value);
                      if (selectedGenerationMode === "video") setVideoModel(event.target.value);
                      if (selectedGenerationMode === "audio") setAudioModel(event.target.value);
                      updateNode(selectedNode.id, { metadata: { ...(selectedNode.metadata || {}), model: event.target.value } });
                    }}
                  >
                    {selectedGenerationMode === "text"
                      ? textModels.map((item) => <option key={item} value={item}>{textModelLabels[item] || item}</option>)
                      : selectedGenerationMode === "image"
                        ...
                        : selectedGenerationMode === "video"
                          ? videoModels.map((item) => <option key={item} value={item}>{textModelLabels[item] || item}</option>)
