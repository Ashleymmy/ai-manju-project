import { ArrowUpRight, Box, Check, Clapperboard, Loader2, Save, WandSparkles } from "lucide-react";
import { useCallback, useEffect, useMemo, useRef, useState } from "react";
import { toast } from "sonner";
import { useLocation } from "wouter";
import { createProject, publicApiError, type Asset, type WorkspaceScope, uploadAsset } from "@/services/api";
...
export default function DirectorDeskView() {
  ...
  const instanceId = useMemo(() => `ai-manju-director-${crypto.randomUUID()}`, []);
  ...
  return (
    <div className="feature-page director-page director-embed-page">
      <div className="feature-title">
        <div><p className="eyebrow">DIRECTOR / EMBED</p><h1>3D 导演台</h1><p>真实嵌入 director-desk，导出的当前帧会进入资产库，并可衍生为画布节点。</p></div>
        <div className="director-actions">
          ...
          <button className="vermilion-button" onClick={() => void sendToCanvas()} disabled={!lastAsset}><WandSparkles size={16} /> 送入画布</button>
        </div>
      </div>
