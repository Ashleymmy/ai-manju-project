export type ApiEnvelope<T> = { success: true; data: T; request_id?: string } | { success: false; error?: string; request_id?: string };
...
// API-ARCHITECTURE.md 的 NEXT_PUBLIC_API_URL 在本 Vite 客户端中以 VITE_API_URL 暴露。
export const API_BASE_URL = (import.meta.env.VITE_API_URL || "http://localhost:3101").replace(/\/$/, "");
...
export function getAuthToken() {
  const preferred = localStorage.getItem(TOKEN_STORE_KEY) === "local" ? localStorage : sessionStorage;
  return preferred.getItem(TOKEN_KEY) || localStorage.getItem(TOKEN_KEY) || sessionStorage.getItem(TOKEN_KEY);
}
...
export async function request<T>(path: string, options: ApiRequestOptions = {}): Promise<T> {
  ...
  try {
    ...
    const responseRequestId = response.headers.get("X-Request-Id") || response.headers.get("x-request-id") || id;
    ...
    if (!response.ok || envelope?.success === false) {
      const message = (envelope && "error" in envelope && envelope.error) || `请求失败（${response.status}）`;
      throw new ApiError(message, response.status, responseRequestId, parsed);
    }
    return envelope && envelope.success === true ? envelope.data as T : parsed as T;
  } catch (error) {
...
export function getCollection(value: unknown): { items: unknown[]; total?: number } {
  if (Array.isArray(value)) return { items: value, total: value.length };
  if (value && typeof value === "object") {
    const record = value as Record<string, unknown>;
    const items = [record.items, record.data, record.projects, record.assets, record.jobs, record.tags].find(Array.isArray) as unknown[] | undefined;
    return { items: items || [], total: typeof record.total === "number" ? record.total : items?.length };
  }
  return { items: [] };
}
