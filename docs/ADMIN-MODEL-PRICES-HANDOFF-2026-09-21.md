...
自动规格继续保留原基础扣费行为，界面展示范围或每秒单价。2026-09-22 按用户澄清修正：只在引用视频时，总积分为 `(有参考视频单价 + 参考视频附加费) × 生成视频秒数`；引用图片、音频保持原视频单价。多个参考视频只加一次单价。H3 480p 默认 30/30/30，可在后台修改，读取旧表时只补新行并保留其余已保存定价。
