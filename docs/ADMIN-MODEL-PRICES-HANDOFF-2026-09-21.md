...
- 视频按模型、分辨率编辑每秒积分。Seedance 1.5 区分无声/有声；其他视频区分无参考/有参考，支持参考时长加价的模型另有附加费。
...
自动规格继续保留原基础扣费行为，界面展示范围或每秒单价。参考视频缺少可靠时长时，时长附加费仍仅展示公式，任务沿用基础规则。本次新增后台编辑不代表这两类场景已经完成精确结算。界面已注明并链接基础规则。
