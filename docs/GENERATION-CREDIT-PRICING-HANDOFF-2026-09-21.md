...
- 2026-09-22 用户进一步确认：图片尺寸或画质为自动（含默认省略）时，按 `(原基础兜底单价 + 参考图张数 × 参考图附加费) × 输出张数` 计费；蒙版排除，活动折扣后汇总向上取整。展示实际报价，不再用价目表范围代替。未匹配模型的自动图片规格也遵循此规则；明确规格但未匹配的模型仍保留原计费。自动视频时长保持原基础规则。
- 2026-09-22 按用户澄清修正：MiniMax H3、Seedance 2.5、Wan 3.0 只在引用视频时加收附加费，按生成视频秒数计算 `(有参考视频单价 + 附加单价) × 生成秒数`；不依赖参考视频时长或数量。图片、音频引用保持无参考视频单价。报价、冻结与成功结算共用此规则，旧任务保持原冻结记录。H3 增加 480p 默认 30/30/30，旧保存配置只补该行，保留其他定价。
